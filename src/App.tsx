...
export default function App() {
  ...
  const [contest, setContest] = useState(false); const [search, setSearch] = useState(''); const [manualSchedule, setManualSchedule] = useState<Schedule | null>(null); const [notice, setNotice] = useState('已加载示例数据，可直接编辑或导入本周表格。'); const fileRef = useRef<HTMLInputElement>(null)
  ...
  return <div className="app-shell">
    <aside className="sidebar"><div className="brand-mark">🐾</div><h1>繁星要塞</h1><p>忍猫指挥部 · 分包台</p><nav><a className="active">要塞分包</a><a>成员管理</a><a>排名与考核</a><a>使用说明</a></nav><div className="sidebar-note">数据只保存在当前浏览器<br />导入即用，导出即走</div></aside>
